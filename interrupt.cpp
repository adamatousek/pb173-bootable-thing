...
InterruptManager *InterruptManager::self;
...
InterruptManager::InterruptManager()
{
    ...
    for( int i = 0; i < N_INTERRUPTS; ++i ) {
        if ( i == 0 || i == 5 || i == 13 || i == 14 )
            ...
            __masys_interrupt_handlers[ i ] = irq_dummy_handler;
        else
...
void InterruptManager::irq_dummy_handler( unsigned intn, unsigned, unsigned )
{
    auto irq = self->intr2irq( intn );
    dbg::sout() << "Got interrupt #" << intn << " (IRQ "
                << +irq << ").\n";
    pic_disable( irq );
    pic_reenable( irq );
}
